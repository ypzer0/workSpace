...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table,
		...
		map.put("className", "Zd"+tableEntity.getClassName());
		map.put("classname", "Zd"+tableEntity.getClassname());
		map.put("pathName", "Zd"+tableEntity.getClassname().toLowerCase());
        ...
        for (String template : templates) {
            ...
            try {
                //添加到zip
				zip.putNextEntry(new ZipEntry(getFileName(template, "Zd"+tableEntity.getClassName(), config.getString("package"))));
                IOUtils.write(sw.toString(), zip, "UTF-8");
                IOUtils.closeQuietly(sw);
                zip.closeEntry();
            } catch (IOException e) {
				throw new RRException("渲染模板失败，表名：" +"Zd"+tableEntity.getTableName(), e);
            }
        }
    }
    ...
    public static String getFileName(String template, String className, String packageName) {
        ...
        if (template.contains("list.html.vm")) {
            return "main" + File.separator + "webapp" + File.separator + "WEB-INF" + File.separator + "page"
            		+ File.separator +"zhuoduan"+ File.separator + className.toLowerCase() + ".html";
        }

        if (template.contains("list.js.vm")) {
			return "main" + File.separator + "webapp" + File.separator + "js" + File.separator
					+"zhuoduan"+ File.separator + className.toLowerCase() + ".js";
        }
