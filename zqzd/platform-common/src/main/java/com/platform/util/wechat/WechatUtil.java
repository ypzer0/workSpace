...
public class WechatUtil {
     ...
     * @throws Exception
     */
    public static String requestOnceCoupon(final String url, String data, Integer plat) throws Exception {
        BasicHttpClientConnectionManager connManager;
        connManager = new BasicHttpClientConnectionManager(
                RegistryBuilder.<ConnectionSocketFactory>create()
                        .register("http", PlainConnectionSocketFactory.getSocketFactory())
        ...
        StringEntity postEntity = new StringEntity(data, "UTF-8");
        httpPost.addHeader("Content-Type", "text/xml");
        httpPost.addHeader("User-Agent", "wxpay sdk java v1.0 " + WeiXinUtil.getMchId(plat));
        httpPost.setEntity(postEntity);
 ...
 * @param url
 * @param data
 * @param type 1 h5类型    2  小程序类型
 * @return
 * @throws Exception
 */
    public static String requestOnceExt(final String url, String data, int type) throws Exception {
        BasicHttpClientConnectionManager connManager;
        connManager = new BasicHttpClientConnectionManager(
                RegistryBuilder.<ConnectionSocketFactory>create()
                        .register("http", PlainConnectionSocketFactory.getSocketFactory())
        ...
        String mchId = "";
        if(type == 1) {
        	mchId = ResourceUtil.getConfigByName("wx.mchId");
        }
        else if(type == 2) {
        	mchId = ResourceUtil.getConfigByName("wxc.mchId");
        }
